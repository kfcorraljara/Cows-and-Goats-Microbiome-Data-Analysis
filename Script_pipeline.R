...
plot(perf.pls$measures$Q2.total$values$comp, perf.pls$measures$Q2.total$values$value, type = "b", xlab = "Components", ylab = "Q2 total")
abline(h=0.0975)
list.keepX <- c(2:10, 15, 20, 25)
tune.splls.MAE <- tune.spls(data_cluster_mixomic, CH4, test.keepX = list.keepX, ncomp = 3, validation = 'Mfold', folds = 5, nrepeat = 10, progressBar = FALSE, measure = 'MAE')
